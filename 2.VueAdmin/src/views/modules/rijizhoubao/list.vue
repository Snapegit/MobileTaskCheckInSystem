<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">日记名称</label>
						<el-input v-model="searchForm.rijimingcheng" placeholder="日记名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">日记类型</label>
						<el-input v-model="searchForm.rijileixing" placeholder="日记类型" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('rijizhoubao','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('rijizhoubao','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('rijizhoubao','按日统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						按日统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('rijizhoubao','按月统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						按月统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('rijizhoubao','按年统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						按年统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('rijizhoubao','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('rijizhoubao','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('rijizhoubao','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="riqiriqiChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="riqiriqiChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="riqiriqiChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var riqiriqiChart1 = echarts.init(document.getElementById("riqiriqiChart1"),'macarons');
        this.$http({
            url: "rijizhoubao/group/riqiriqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                riqiriqiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    riqiriqiChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var riqiriqiChart2 = echarts.init(document.getElementById("riqiriqiChart2"),'macarons');
        this.$http({
            url: "rijizhoubao/group/riqiriqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                riqiriqiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    riqiriqiChart2.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var riqiriqiChart3 = echarts.init(document.getElementById("riqiriqiChart3"),'macarons');
        this.$http({
            url: "rijizhoubao/group/riqiriqi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                riqiriqiChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    riqiriqiChart3.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.rijileixing!='' && this.searchForm.rijileixing!=undefined){
            params['rijileixing'] = '%' + this.searchForm.rijileixing + '%'
          }
    params['sort'] = 'riqiriqi';
    params['order'] = 'desc';
      this.$http({
	...
	rijizhoubaostatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'rijizhoubao/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
