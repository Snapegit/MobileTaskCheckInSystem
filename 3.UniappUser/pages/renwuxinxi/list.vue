...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"linear-gradient(0deg, rgba(255,255,255,1) 80%, rgba(34,56,96,1) 98%, rgba(34,56,96,1) 100%),#fff","flexDirection":"column","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			...
			<view :style='{"alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"10"}'>
				<scroll-view scroll-x="true" class="category-one" :style='{"margin":"40rpx 0 0","whiteSpace":"nowrap","background":"none","display":"flex","width":"calc(100% - 48rpx)","justifyContent":"center","height":"auto","order":"0"}'>
					<view :class='categoryName === item.renwufenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.renwufenlei)">{{item.renwufenlei}}</view>
				</scroll-view>
			...
			<view class="list" :style='{"padding":"24rpx","margin":"136rpx 0 0","overflow":"hidden","background":"#fff","width":"100%","height":"auto","order":"50"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 20rpx","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('renwuxinxi','修改')) || (!userid && isAuthFront('renwuxinxi','修改'))" @click.stop="onUpdateTap(product)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('renwuxinxi','删除')) || (!userid && isAuthFront('renwuxinxi','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 0px 4rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"128rpx","right":"128rpx","borderRadius":"100%","background":"#ff6b5a","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('renwuxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('renwufenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('renwufenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,renwufenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.renwufenlei = '%' + this.categoryName + '%'
				}
				...
				if(indexQueryCondition) {
					params['renwumingcheng'] = '%' + indexQueryCondition + '%';
