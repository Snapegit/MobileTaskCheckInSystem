...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"linear-gradient(0deg, rgba(255,255,255,1) 80%, rgba(34,56,96,1) 98%, rgba(34,56,96,1) 100%),#fff","flexDirection":"column","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"padding":"0px","margin":"0 auto","borderRadius":"16rpx","background":"none","display":"flex","width":"calc(100% - 48rpx)","height":"auto"}'>
				...
				<view :style='{"margin":"0","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 80rpx","color":"#333","borderRadius":"16rpx 0 0 16rpx","background":"#fff","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.shangpinmingcheng" type="text" placeholder="商品名称" ></input>
				</view>
				<view :style='{"margin":"0","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing01" :style='{"color":"#0060c8","borderRadius":"16rpx 0 0 16rpx","left":"0px","textAlign":"center","background":"#fff","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 80rpx","color":"#333","borderRadius":"16rpx 0 0 16rpx","background":"#fff","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.shangpinfenlei" type="text" placeholder="商品分类" ></input>
				</view>
				<view :style='{"margin":"0","flex":"1","position":"relative"}' v-if="queryIndex==2" class="search-form round">
					<text class="icon iconfont icon-fangdajing01" :style='{"color":"#0060c8","borderRadius":"16rpx 0 0 16rpx","left":"0px","textAlign":"center","background":"#fff","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 80rpx","color":"#333","borderRadius":"16rpx 0 0 16rpx","background":"#fff","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.shangpinpinpai" type="text" placeholder="商品品牌" ></input>
				</view>
			...
			<view class="list" :style='{"padding":"24rpx","margin":"136rpx 0 0","overflow":"hidden","background":"#fff","width":"100%","height":"auto","order":"50"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 20rpx","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"200rpx","objectFit":"cover","borderRadius":"8rpx","display":"block","clipPath":"polygon(10% 0, 100% 0%, 90% 100%, 0% 100%)","height":"240rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.shangpintupian)" :src="product.shangpintupian.split(',')[0]"></image>
					<image :style='{"width":"200rpx","objectFit":"cover","borderRadius":"8rpx","display":"block","clipPath":"polygon(10% 0, 100% 0%, 90% 100%, 0% 100%)","height":"240rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.shangpintupian?baseUrl+product.shangpintupian.split(',')[0]:''"></image>
					...
					<view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"12rpx 0 12rpx 20rpx","margin":"0","background":"linear-gradient(30deg, rgba(246,249,255,1) 0%, rgba(239,244,255,1) 100%),#f1f6ff","clipPath":"polygon(4% 0, 100% 0%, 96% 100%, 0% 100%)","height":"240rpx"}'>
						<view class="col3 f30 elip mb15" :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"28rpx","color":"#333"}'>{{product.shangpinmingcheng}}</view>
						<view class="col3 f30 elip mb15" :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"28rpx","color":"#333"}'>商品数量:{{product.shangpinshuliang}}</view>
						<view class="col3 f30 elip mb15" :style='{"width":"100%","padding":"0 20rpx","margin":"0","lineHeight":"48rpx","fontSize":"28rpx","color":"#333"}'>商品积分:{{product.shangpinjifen}}</view>
					...
					<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jifenshangpin','修改')) || (!userid && isAuthFront('jifenshangpin','修改'))" @click.stop="onUpdateTap(product)">
		...
		<button :style='{"border":"0","boxShadow":"0 0px 4rpx rgba(0,0,0,.3)","color":"rgb(255, 255, 255)","bottom":"128rpx","right":"128rpx","borderRadius":"100%","background":"#ff6b5a","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('jifenshangpin','新增')" class="add-btn" @click="onAddTap()">新增</button>
